...
public class GroupWriteService {
    ...
    public void groupWrite(UserDetailsImpl userDetails, GroupWriteRequestDto requestDto) throws IOException {
        ...
        ExcelFile excelFile = ExcelFile.builder()
                ...
                .build();
        excelFileRepository.save(excelFile); // 엑셀 파일 저장
        ...
        List<ExcelData> excelDataList = dataList.stream().map(ExcelData::new).collect(Collectors.toList());
        ...
        for (PropertyDto property : properties) {
            CustomerProperty customerProperty = CustomerProperty.builder()
    ...
    // group 수정
    @Transactional
    public void groupUpdate(Long groupId, GroupWriteRequestDto requestDto) throws IOException {
        CustomerGroup findGroup = customerGroupRepository.findById(groupId).get();
        ...
        if (!multipartFile.isEmpty()) {
            ...
            for (int i = 0; i < requestDto.getProperties().size(); i++) {
                CustomerProperty customerProperty = CustomerProperty.builder()
                        .propertyValue(requestDto.getProperties().get(i).getPropertyValue())
                        .propertyName(requestDto.getProperties().get(i).getPropertyName())
